...
def createGUI():
    ...
    titl = tkinter.Label(mainFrame,text="CONSTRUCIÓN DE AFD EN BASE A UN EXPRESIÓN REGULAR")
    ...
    validStringEntry.grid(column= 1,row=2,ipadx=2, ipady=2, padx=5, pady=10,columnspan=2)
    validStringEntry.config(state='disabled',width=25)

    #btn


    genAFDButton = tkinter.Button(mainFrame, text="Generar AFD",command=genAFD)
    ...
    validStringButton = tkinter.Button(mainFrame, text="Validar", command=valid)
    ...
    clearButton.grid(column=0, row=3, ipadx=2, ipady=2, padx=10, pady=10,columnspan=4  )
...
def genAFD():
    ...
    error = ''
    for x in reguExpresion.get():
        if x == '.' :
            error = x
            break
    if reguExpresion.get()=='' or error == '.':
        ...
        try:
            ...
            messagebox.showinfo("ERROR:", "Expresión invalida (missing parenthesis)")
...
def valid():
    if reguExpresion.get()=='':
        ...
        messagebox.showinfo("Pertenece la cadena a la expresión regular?",ddfa_regex)
        validString.set("")
        pass
